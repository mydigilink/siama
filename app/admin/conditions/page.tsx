...
export default function AdminConditionsPage() {
  ...
  const [filters, setFilters] = useState<ConditionSearchParams>({
    page: 1,
    limit: PAGE_SIZE,
    sortBy: 'name',
    sortOrder: 'asc',
  ...
  const [formData, setFormData] = useState<ConditionPayload>({
    name: '',
    image: '',
    status: true,
    category: '',
  ...
  const loadConditions = useCallback(async () => {
    try {
      setLoading((prev) => prev && conditions.length === 0);
      setSearchLoading(true);
      const response = await conditionsApi.search({ ...filters, search: searchTerm || filters.search });
      if (response.status === 'success' && response.data) {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- omit conditions.length to avoid refetch when data arrives
  }, [filters, searchTerm]);
  ...
  const handleFilterChange = (key: keyof ConditionSearchParams, value: any) => {
    setFilters({
      ...filters,
      [key]: value || undefined,
      page: 1,
  ...
  const openEdit = (cond: Condition) => {
    ...
    setFormData({
      name: cond.name || '',
      image: cond.image || '',
      status: cond.status ?? true,
      category: typeof cond.category === 'string' ? cond.category : (cond.category as any)?._id || '',
      slug: cond.slug || '',
    });
    setShowForm(true);
  };
  ...
  const submitForm = async () => {
    try {
      setSubmitting(true);
      const payload: ConditionPayload = { ...formData };
      if (isEdit && selectedCondition?._id) {
        await conditionsApi.update(selectedCondition._id, payload);
      } else {
  ...
  const toggleStatus = async (id: string) => {
    try {
      await conditionsApi.toggleStatus(id);
      await loadConditions();
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50">
      ...
      <header className="bg-white/90 backdrop-blur border-b border-gray-200 h-16 flex items-center justify-between px-4 sticky top-0 z-20 shadow-sm">
        <div className="flex flex-col">
          ...
          <p className="text-xs text-gray-500">Manage conditions (status change, create/edit).</p>
        </div>
      ...
      <main className="p-4 space-y-4">
        ...
        <div className="bg-white/90 backdrop-blur rounded-xl shadow-sm border border-gray-100 p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              onClick={() => {
                setFilters({ page: 1, limit: PAGE_SIZE, sortBy: 'name', sortOrder: 'asc' });
                setSearchTerm('');
              }}
            ...
            <table className="w-full min-w-[1040px]">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {searchLoading ? (
                  ...
                  conditions.map((cond) => (
                    <tr key={cond._id} className="hover:bg-indigo-50/40 transition-colors border-b border-gray-100 align-middle">
                      <td className="px-5 py-3 whitespace-nowrap align-middle">
                        <div className="text-sm font-semibold text-gray-900 leading-5">{cond.name}</div>
                        {cond.slug && <div className="text-xs text-gray-500">/{cond.slug}</div>}
                      </td>
                      <td className="px-5 py-3 whitespace-nowrap align-middle">
                        <div className="text-sm text-gray-900 leading-5">
                          {typeof cond.category === 'string'
                            ? categories.find((c) => c._id === cond.category)?.name || '—'
                            : (cond.category as any)?.name || '—'}
                        </div>
                      </td>
                      <td className="px-5 py-3 whitespace-nowrap align-middle">
                        <span className={`inline-flex px-2 py-0.5 text-xs font-semibold rounded-full ${cond.status ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-gray-100 text-gray-700 border border-gray-200'}`}>
                          {cond.status ? 'Active' : 'Inactive'}
                        </span>
                      </td>
                      <td className="px-5 py-3 whitespace-nowrap text-sm text-gray-500 align-middle">
                        {cond.updatedAt ? new Date(cond.updatedAt).toLocaleDateString() : '—'}
                      </td>
                      <td className="px-5 py-3 whitespace-nowrap align-middle flex items-center space-x-2">
                        <button
                          onClick={() => openDetail(cond._id)}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-40 px-4">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-gray-100 overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gradient-to-r from-amber-50 via-yellow-50 to-white">
              <div>
                <p className="text-sm font-semibold text-gray-900">{isEdit ? 'Edit Condition' : 'New Condition'}</p>
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gradient-to-r from-gray-50 via-white to-gray-50">
              <div>
                <p className="text-sm font-semibold text-gray-900">
                  {selectedCondition?.name || 'Condition detail'}
                </p>
                <p className="text-xs text-gray-500">
                  {selectedCondition?.slug ? `/${selectedCondition.slug}` : '—'}
                </p>
              </div>
              <button
                onClick={closeDetail}
              ...
              {detailLoading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-700">
                  <div className="space-y-1">
                    ...
                    <p className="font-semibold text-gray-900">{selectedCondition?.name || 'N/A'}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-xs text-gray-500">Slug</p>
                    <p className="font-semibold text-gray-900">{selectedCondition?.slug || 'N/A'}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-xs text-gray-500">Category</p>
                    <p className="font-semibold text-gray-900">
                      {typeof selectedCondition?.category === 'string'
                        ? categories.find((c) => c._id === selectedCondition.category)?.name || 'N/A'
                        : (selectedCondition?.category as any)?.name || 'N/A'}
                    </p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-xs text-gray-500">Status</p>
                    <span className={`inline-flex px-2 py-0.5 text-xs font-semibold rounded-full capitalize ${selectedCondition?.status ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-gray-100 text-gray-700 border border-gray-200'}`}>
                      {selectedCondition?.status ? 'Active' : 'Inactive'}
                    </span>
                  </div>
                  <div className="space-y-1 sm:col-span-2">
                    <p className="text-xs text-gray-500">Image</p>
                    <p className="font-semibold text-gray-900 break-all">{selectedCondition?.image || 'N/A'}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-xs text-gray-500">Created</p>
                    <p className="font-semibold text-gray-900">
                      {selectedCondition?.createdAt ? new Date(selectedCondition.createdAt).toLocaleString() : 'N/A'}
                    </p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-xs text-gray-500">Updated</p>
                    <p className="font-semibold text-gray-900">
                      {selectedCondition?.updatedAt ? new Date(selectedCondition.updatedAt).toLocaleString() : 'N/A'}
                    </p>
                  </div>
                </div>
              )}
